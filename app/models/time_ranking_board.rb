class TimeRankingBoard < RankingBoard
  has_many :rank_items, class_name: 'TimeRankItem', foreign_key: :ranking_board_id
  ...
  def add_item!(params)
    ...
    # 新規レコード、または過去のスコアの方が高い場合にスコアを保存
  ...
  def top_ranking(uid)
    last_rank = nil
    last_score = nil
    rank_items.order(score: :asc, updated_at: :asc).limit(MAX_LIST_COUNT).map.with_index do |item, i|
      obj = {
  ...
  def recent_score(uid)
    item = recent_items.find_by(uid:).extend(Decorator::TimeScore)

    return {} unless item

    # プレイヤーの最新スコアより低いレコードを取得して順位を算出
    rank = rank_items.where("score < ?", item.score).count + 1
